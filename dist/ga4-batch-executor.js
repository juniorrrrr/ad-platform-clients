...
export class GA4BatchExecutor {
     ...
     */
    async executePlan(plan, propertyId, accessToken, dateRanges) {
        ...
        const executions = plan.queries.map(async (query) => {
            try {
                const { result, retries, requests } = await this.executeQueryWithRetry(query, propertyId, accessToken, dateRanges);
     ...
     */
    async executeQueryWithRetry(query, propertyId, accessToken, dateRanges) {
        let lastError = null;
        let attempts = 0;
        for (let i = 0; i <= this.options.maxRetries; i++) {
            attempts++;
            try {
                const result = await this.executeQuery(query, propertyId, accessToken, dateRanges);
     ...
     */
    async executeQuery(query, propertyId, accessToken, dateRanges) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(json.error?.message ?? `GA4 API error (${response.status})`);
            }
            return json;
        }
        catch (err) {
    ...
    isRetryable(error) {
        ...
        if (msg.includes("invalid argument"))
            return false;
        if (msg.includes("unrecognized field"))
            return false;
