...
interface IgBusiness {
  id: string;
  name: string;
}

/** Follows `paging.next` so accounts with >100 pages don't silently lose the tail. */
async function paginatedIgFetch<T>(url: string): Promise<T[]> {
  const out: T[] = [];
  let next: string | undefined = url;
  while (next) {
    const json: { data?: T[]; paging?: { next?: string } } = await igFetch(next);
    out.push(...(json.data ?? []));
...
export async function graphBatch<T>(
  ...
  const chunks: string[][] = [];
  for (let i = 0; i < relativeUrls.length; i += 50) chunks.push(relativeUrls.slice(i, i + 50));
      ...
      withRateLimitBackoff(async () => {
        ...
        if (!Array.isArray(json)) {
          throw new GraphApiError(
            json.error?.message ?? `Batch request failed (HTTP ${res.status})`,
            json.error?.code ?? 0,
        ...
        return json.map((item) => {
          ...
          try {
            return JSON.parse(item.body) as T;
          } catch {
            return null;
          }
        });
...
async function listOwnedBusinesses(accessToken: string): Promise<IgBusiness[]> {
  const enc = encodeURIComponent(accessToken);
  return paginatedIgFetch<IgBusiness>(
    `${IG_API}/me/businesses?fields=id,name&limit=100&access_token=${enc}`,
  );
}
 ...
 */
export async function listLinkedPages(accessToken: string): Promise<LinkedPagesResult> {
  const enc = encodeURIComponent(accessToken);
  const directPages = await paginatedIgFetch<IgPage>(
    `${IG_API}/me/accounts?fields=id,name,instagram_business_account,access_token,tasks&limit=100&access_token=${enc}`,
  );

  const warnings: string[] = [];
  let businesses: IgBusiness[] = [];
  try {
    businesses = await listOwnedBusinesses(accessToken);
  } catch (err) {
    warnings.push(
      `Descoberta de Business Managers falhou: ${err instanceof Error ? err.message : String(err)}`,
    );
  }

  const businessPages: IgPage[] = [];
  if (businesses.length > 0) {
    ...
    try {
      const results = await graphBatch<{ data?: IgPage[] }>(accessToken, relativeUrls);
      let failedCount = 0;
      for (const result of results) {
        if (result?.data) businessPages.push(...result.data);
        else failedCount++;
      }
      if (failedCount > 0) {
        warnings.push(
          `${failedCount} de ${relativeUrls.length} consultas de Business Manager (owned_pages/client_pages) não retornaram dados. Permissão insuficiente para aquele ativo específico ou o ativo não existe mais.`,
        );
      }
    } catch (err) {
      warnings.push(
        `Descoberta de páginas via Business Manager falhou: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }

  const byId = new Map<string, IgPage>();
  for (const page of [...directPages, ...businessPages]) byId.set(page.id, page);
...
export async function getIgAccountInfo(
  ...
  const fields =
    "id,name,username,profile_picture_url,followers_count,media_count,biography,website";
  const enc = encodeURIComponent(accessToken);
  const url = `${IG_API}/${igAccountId}?fields=${fields}&access_token=${enc}`;
  return igFetch<IgAccountInfo>(url);
}
 ...
 */
export async function getIgAccountInfoBatch(
  igAccountIds: string[],
  accessToken: string,
): Promise<Map<string, IgAccountInfo>> {
  const fields =
    "id,name,username,profile_picture_url,followers_count,media_count,biography,website";
  const results = await graphBatch<IgAccountInfo>(
    accessToken,
    igAccountIds.map((id) => `${id}?fields=${fields}`),
  );
  const map = new Map<string, IgAccountInfo>();
...
export async function getIgAccountInsights(
  ...
  type RawTotalValueSeries = { name: string; period: string; total_value?: { value: number } };
  const allSeries: IgInsightSeries[] = [];
  ...
  // disso.
  async function fetchChunkReach(
    chunkStart: number,
    chunkEnd: number,
    chunkEndIso: string,
  ): Promise<IgInsightSeries[]> {
    const totalValueUrl =
      `${IG_API}/${igAccountId}/insights` +
      `?metric=reach&period=day&metric_type=total_value&since=${chunkStart}&until=${chunkEnd}` +
      `&access_token=${enc}`;
    try {
      const json = await igFetch<{ data?: RawTotalValueSeries[] }>(totalValueUrl);
      const raw = json.data?.[0];
      if (!raw?.total_value) throw new Error("resposta sem total_value");
      return [{ name: "reach", period: raw.period, values: [{ value: raw.total_value.value, end_time: chunkEndIso }] }];
    } catch (e) {
      console.warn(
        "[instagram-api] reach com metric_type=total_value falhou, caindo pra period=day somado (menos preciso, contas únicas repetidas em dias diferentes contam 2x):",
        (e as Error).message,
      );
      const fallbackUrl =
        `${IG_API}/${igAccountId}/insights?metric=reach&period=day&since=${chunkStart}&until=${chunkEnd}&access_token=${enc}`;
      try {
        const json = await igFetch<{ data?: IgInsightSeries[] }>(fallbackUrl);
        return json.data ?? [];
      } catch (e2) {
        console.warn("[instagram-api] fallback de reach (period=day) também falhou:", (e2 as Error).message);
        return [];
      }
    }
  }
  ...
  for (const chunk of chunks) {
    ...
    const normalize = (data: unknown[]): IgInsightSeries[] =>
      (data as RawTotalValueSeries[]).map((s) => ({
        name: s.name,
        period: s.period,
    ...
    const [reachSeries, totalValueJson, viewsJson] = await Promise.all([
      ...
      igFetch<{ data?: IgInsightSeries[] }>(totalValueUrl),
      igFetch<{ data?: IgInsightSeries[] }>(viewsUrl).catch((e) => {
        console.warn(
          "[instagram-api] account-level views (impressions replacement) failed:",
          (e as Error).message,
        );
        return { data: [] as IgInsightSeries[] };
      }),
    ]);

    allSeries.push(
      ...reachSeries,
      ...normalize(totalValueJson.data ?? []),
  ...
  if (followerSince <= untilTs) {
    const followerUrl =
      ...
      `&access_token=${enc}`;
    try {
      const followerJson = await igFetch<{ data?: IgInsightSeries[] }>(followerUrl);
      allSeries.push(...(followerJson.data ?? []));
    } catch (e) {
      console.warn("[instagram-api] follower_count insights failed:", (e as Error).message);
    }
  }
...
export async function getIgMedia(
  ...
  while (url && allMedia.length < maxItems) {
    const json = await igFetch<{ data?: IgMedia[]; paging?: { next?: string } }>(url);
...
export async function getIgStories(igAccountId: string, accessToken: string): Promise<IgMedia[]> {
  const fields = "id,media_type,timestamp,caption,thumbnail_url,media_url,permalink";
  const enc = encodeURIComponent(accessToken);
  const url = `${IG_API}/${igAccountId}/stories?fields=${fields}&limit=100&access_token=${enc}`;
  try {
    const json = await igFetch<{ data?: IgMedia[] }>(url);
    return json.data ?? [];
  } catch {
    return [];
  }
...
function mediaInsightsMetrics(mediaType: string, createdAt: string): string {
  const includeImpressions = Date.parse(createdAt) < IMPRESSIONS_MEDIA_CUTOFF;
  const baseMetrics =
    (includeImpressions ? "impressions," : "") +
...
export async function getMediaInsightsBatch(
  items: Array<{ id: string; mediaType: string; createdAt: string }>,
  accessToken: string,
): Promise<Map<string, IgMediaInsights>> {
  const results = await graphBatch<{
    data?: Array<{ name: string; values?: Array<{ value: number }>; value?: number }>;
  }>(
    accessToken,
    items.map(
      (item) =>
        `${item.id}/insights?metric=${mediaInsightsMetrics(item.mediaType, item.createdAt)}`,
    ),
  );
  const map = new Map<string, IgMediaInsights>();
  items.forEach((item, i) => {
    const raw = results[i]?.data;
    if (!raw) return;
    const result: IgMediaInsights = {};
    for (const m of raw) {
      const val = m.values?.[0]?.value ?? (m.value as number | undefined) ?? 0;
...
// `audience_gender_age`, `audience_city` and `audience_country` are confirmed
// rejected by the Graph API for this account ("(#100) metric[0] must be one of
// the following values: ... follower_demographics ..." — none of the three
// legacy names appear). Meta replaced them with `follower_demographics`, which
// requires `metric_type=total_value` and a `breakdown` dimension, and returns a
// `total_value.breakdowns[].results[]` shape instead of the old `values[].value`
// map. We fetch one call per breakdown dimension and normalize back into the
// `{ [gender.age | city | country]: pct }` map shape the rest of the pipeline
// already expects, so downstream parsing in instagram.functions.ts is unchanged.
type RawBreakdownResult = { dimension_values: string[]; value: number };
type RawBreakdownSeries = {
  name: string;
  total_value?: {
    value: number;
    breakdowns?: Array<{ dimension_keys: string[]; results: RawBreakdownResult[] }>;
  };
};

async function fetchFollowerDemographic(
  igAccountId: string,
  accessToken: string,
  breakdown: string,
): Promise<RawBreakdownResult[]> {
  const enc = encodeURIComponent(accessToken);
  const url =
    `${IG_API}/${igAccountId}/insights` +
    ...
    `&access_token=${enc}`;
  try {
    const json = await igFetch<{ data?: RawBreakdownSeries[] }>(url);
    return json.data?.[0]?.total_value?.breakdowns?.[0]?.results ?? [];
  } catch (e) {
    console.warn(
      `[instagram-api] follower_demographics(${breakdown}) failed:`,
      (e as Error).message,
    );
    return [];
  }
...
export async function getIgAudience(
  igAccountId: string,
  accessToken: string,
): Promise<IgAudienceEntry[]> {
  ...
  const toPctMap = (
    results: RawBreakdownResult[],
    keyFromValues: (v: string[]) => string,
  ): Record<string, number> => {
    const total = results.reduce((s, r) => s + r.value, 0) || 1;
    const map: Record<string, number> = {};
    for (const r of results) map[keyFromValues(r.dimension_values)] = (r.value / total) * 100;
    return map;
  };

  const entries: IgAudienceEntry[] = [];
...
export async function getOnlineFollowers(
  ...
  try {
    const json = await igFetch<{
      data?: Array<{
        name: string;
        values: Array<{ value: Record<string, Record<string, number>> }>;
      }>;
    }>(url);
