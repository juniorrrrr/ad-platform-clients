...
import type { GA4ReportResponse, GA4ReportRow } from "./ga4-batch-executor";
...
export class GA4ResultMerger {
   ...
   */
  private mergeMetricOnlyQueries(context: MergeContext): GA4ReportResponse {
    // Collect all metrics in order
    const allMetrics: { name: string }[] = [];
    const allMetricIndices = new Map<string, number>();
    ...
    let mergedRow: GA4ReportRow | null = null;
   ...
   */
  private mergeDimensionalQueries(context: MergeContext): GA4ReportResponse {
    // Collect all dimensions and metrics in order
    const allDimensions: { name: string }[] = [];
    const allMetrics: { name: string }[] = [];
    ...
    const rowsByDimensionKey = new Map<string, GA4ReportRow>();
    ...
    for (const query of context.queries) {
      ...
      for (const queryRow of result.rows) {
        ...
        const dimKey = this.buildDimensionKey(
          query,
          queryRow,
          dimensionIndices,
        );
  ...
  static validate(context: MergeContext): void {
    ...
    for (const query of context.queries.slice(1)) {
      ...
      if (JSON.stringify(firstDims) !== JSON.stringify(queryDims)) {
        throw new Error(
          "Cannot merge queries with different dimensions: " +
    ...
    if (context.queries[0].dimensions.length > 0) {
      ...
      for (const query of context.queries.slice(1)) {
        if (query.limit !== firstLimit) {
          console.warn(
            "[GA4] Warning: Queries have different row limits. Results may be inconsistent."
          );
        }
        if (JSON.stringify(query.orderBys) !== firstOrder) {
          console.warn(
            "[GA4] Warning: Queries have different ordering. Results may be inconsistent."
          );
        }
      }
    }
