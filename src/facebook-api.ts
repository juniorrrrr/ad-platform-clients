...
export async function getFbPageInfo(pageId: string, pageAccessToken: string): Promise<FbPageInfo> {
...
export async function getFbPageInsights(
  ...
  since: string,
  until: string,
): Promise<{ series: FbInsightSeries[]; errors: string[] }> {
  ...
  const series: FbInsightSeries[] = [];
  const errors: string[] = [];

  for (const metrics of metricGroups) {
    const url =
      `${FB_API}/${pageId}/insights` +
      `?metric=${metrics.join(",")}&period=day&since=${sinceTs}&until=${untilTs}` +
      `&access_token=${enc}`;
    try {
      const json = await fbFetch<{ data?: FbInsightSeries[] }>(url);
      ...
      errors.push(`Métricas (${metrics.join(",")}): ${(e as Error).message}`);
    }
  }
...
export async function getFbPagePosts(
  pageId: string,
  pageAccessToken: string,
  since: string,
  until: string,
  ...
  while (url && allPosts.length < maxItems) {
    const json = await fbFetch<{ data?: FbPost[]; paging?: { next?: string } }>(url);
 ...
 */
export async function getFbPageAudience(
  pageId: string,
  pageAccessToken: string,
): Promise<{ entries: FbAudienceEntry[]; error: string | null }> {
  const enc = encodeURIComponent(pageAccessToken);
  ...
  try {
    const json = await fbFetch<{ data?: FbAudienceEntry[] }>(url);
 ...
 */
export async function getFbPostInsightsBatch(
  postIds: string[],
  pageAccessToken: string,
): Promise<Map<string, FbPostInsights>> {
  // post_impressions → post_media_view, mesma leva de renomeação de
  // page_impressions → page_media_view (ver getFbPageInsights acima).
  const results = await graphBatch<{ data?: Array<{ name: string; values?: Array<{ value: number }> }> }>(
    pageAccessToken,
    postIds.map((id) => `${id}/insights?metric=post_media_view`),
  );
  const map = new Map<string, FbPostInsights>();
  postIds.forEach((id, i) => {
    const raw = results[i]?.data;
    if (!raw) return;
    const out: FbPostInsights = {};
    for (const m of raw) {
      const val = m.values?.[0]?.value;
      if (val === undefined) continue;
      if (m.name === "post_media_view") out.impressions = val;
    }
    map.set(id, out);
  });
  return map;
 ...
 * Publicações.
 */
export async function getFbPostEngagementBatch(
  postIds: string[],
  pageAccessToken: string,
): Promise<Map<string, FbPostEngagement>> {
  const results = await graphBatch<{ likes?: { summary?: { total_count: number } }; comments?: { summary?: { total_count: number } } }>(
    pageAccessToken,
    postIds.map((id) => `${id}?fields=likes.summary(true),comments.summary(true)`),
  );
  const map = new Map<string, FbPostEngagement>();
  postIds.forEach((id, i) => {
    const raw = results[i];
    if (!raw) return;
    map.set(id, {
