...
export type NormalizedAdAccount = {
  accountId:     string;   // always "act_XXXXX"
  name:          string;
  currency:      string;
...
async function fetchAllPages<T>(
  ...
  while (nextUrl) {
    ...
    if (!res.ok || json.error) {
      ...
      if (page === 0 && throwOnFirstError) throw new Error(msg);
...
async function fetchSafe<T>(url: string, label: string): Promise<T[]> {
  try {
    return await fetchAllPages<T>(url, label, false);
  } catch (err) {
...
// ─── Normalise ────────────────────────────────────────────────────────────────
function normalise(raw: RawAccount, bizOverride?: RawBusiness): NormalizedAdAccount {
  // `id` always comes back as "act_XXXXX"; `account_id` is the numeric-only fallback
  const accountId =
    raw.id?.startsWith("act_")
      ? raw.id
      : raw.id
      ? `act_${raw.id}`
...
// ─── Main export ──────────────────────────────────────────────────────────────
export async function fetchAllMetaAdAccounts(token: string): Promise<NormalizedAdAccount[]> {
  const enc = encodeURIComponent(token);

  // ── Step 1: direct accounts (THROWS on failure — primary source) ───────────
  const directRaw = await fetchAllPages<RawAccount>(
    `https://graph.facebook.com/${META_API_VERSION}/me/adaccounts?fields=${FIELDS}&limit=200&access_token=${enc}`,
    "me/adaccounts",
    true,
  );
  ...
  const businesses = await fetchSafe<RawBusiness>(
    `https://graph.facebook.com/${META_API_VERSION}/me/businesses?fields=id,name&limit=50&access_token=${enc}`,
    "me/businesses",
  );
  ...
  const businessGroups = await Promise.all(
    businesses.map(async (biz) => {
      const [owned, clients] = await Promise.all([
        fetchSafe<RawAccount>(
          `https://graph.facebook.com/${META_API_VERSION}/${biz.id}/owned_ad_accounts?fields=${FIELDS}&limit=200&access_token=${enc}`,
          `biz:${biz.name}/owned`,
        ),
        fetchSafe<RawAccount>(
          `https://graph.facebook.com/${META_API_VERSION}/${biz.id}/client_ad_accounts?fields=${FIELDS}&limit=200&access_token=${enc}`,
          `biz:${biz.name}/clients`,
        ),
      ]);
